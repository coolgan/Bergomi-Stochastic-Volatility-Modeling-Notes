...
- 实验 1：香草、奇异期权对波动率动态设定的敏感性实验（Vol Dynamic Sensitivity）
- 实验 2：同一初始微笑下的模型动态与产品定价差异（Model Dynamic Comparison）
- 实验 3：随机波动率微笑微扰近似与 ATMF 指标实验报告（SV Smile Perturbation）
- 实验 4：期权动态盈亏拆解（一）：基于香草期权的仿真实验（PnL Accounting Vanilla）
- 实验 5：期权动态盈亏拆解（二）：基于鲨鱼鳍期权的仿真实验（PnL Accounting Shark-Fin）
...
- `index.html`：章节入口页；
- `chapter-*.html`：各章笔记页面；
- `experiment-*.html`：数值实验页面；
- `assets/`：CSS、页面脚本、KaTeX 样式和字体；
- `images/`：笔记中引用的书稿图表；
- `experiments/`：数值实验图表与可下载脚本。
